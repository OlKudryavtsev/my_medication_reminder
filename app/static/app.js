  ...
  // ===== v34: improved assignments/courses/schedule UI =====
  ...
  function courseNeedPanel(g){
    const unit=escapeHtml(g.consume_unit_name||doseParts(g.dose).unit||'шт');
    ...
    return `<div class="needPanel">Нужно на курс: ${fmtNum(g.planned_units_total||0)} ${unit}<br>Осталось нужно: ${fmtNum(g.remaining_need_units||0)} ${unit}<br>В аптечке: ${stock} ${unit} · ${ok}</div>`;
  }
  function courseCourseHeader(g,key){return `<div class="courseGroupHeader" onclick="toggleCourseGroup('${key}')"><div><div class="courseTitle">${escapeHtml(g.name||'Лекарство')}</div><div class="courseDuration">${escapeHtml(durationText(g.start_date,g.end_date))} · ${g.active_all?'активен':(g.active_any?'частично активен':'не начат')}</div></div><div class="chev" id="chev_${key}">＋</div></div>`}
  ...
  function courseGroupBody(g,key){
    ...
    const times=[...new Set((g.entries||[]).map(slotText))].join(', ');
    const analogs=(g.analogs||'').trim();
    const route=(g.administration_route||g.label||'').trim();
    ...
    return `<div id="courseBody_${key}" class="courseGroupBody hidden"><div class="infoGrid"><div class="infoPill"><div class="k">Формат</div><div class="v">${escapeHtml(form||'—')}</div></div><div class="infoPill"><div class="k">Доза</div><div class="v">${escapeHtml(g.dose||'—')}</div></div><div class="infoPill"><div class="k">Способ</div><div class="v">${escapeHtml(route||'—')}</div></div><div class="infoPill"><div class="k">Период</div><div class="v">${escapeHtml(period)}</div></div><div class="infoPill"><div class="k">Периодичность</div><div class="v">${escapeHtml((g.entries||[]).length+' раз(а), '+(g.recurrence_type==='daily'?'ежедневно':g.recurrence_type||''))}</div></div><div class="infoPill"><div class="k">Прием</div><div class="v">${escapeHtml(times||'—')}</div></div></div>${analogs?`<div class="infoPill" style="margin-top:8px"><div class="k">Аналоги из назначения</div><div class="v">${escapeHtml(analogs)}</div></div>`:''}${courseNeedPanel(g)}<div class="adminActions">${inactiveActions}<button class="gray" onclick="openCourseMedicineForm(${g.course_id}, null, null, '${key}')">Изменить</button><button class="danger" onclick="deleteScheduleGroup('${g.ids.join(',')}')">Удалить</button></div></div>`;
  }
  ...
  function groupCourseItems(items){
    const map=new Map();
    (items||[]).forEach(it=>{
      const key=[it.course_id||'',(it.name||'').toLowerCase(),it.dose||'',it.start_date||'',it.end_date||'',it.recurrence_type||'',it.recurrence_interval_days||'',it.weekdays||'',it.specific_dates||'',it.timing_template||''].join('|');
      if(!map.has(key)) map.set(key,{...it, ids:[], entries:[], planned_units_total:0, remaining_need_units:0, taken_units:0, active_any:false, active_all:true});
      const g=map.get(key);
      g.ids.push(it.id); g.entries.push(it);
      ...
      if(g.inventory_quantity==null && it.inventory_quantity!=null)g.inventory_quantity=it.inventory_quantity;
      g.consume_unit_name = g.consume_unit_name || it.consume_unit_name || doseParts(it.dose).unit || 'шт';
      g.active_any = g.active_any || !!it.active; g.active_all = g.active_all && !!it.active;
  ...
  function recognizeBoxHtml(){return `<button class="collapseToggle" onclick="recognizeAssignmentOpen=!recognizeAssignmentOpen;loadCourses()"><span>Распознать назначение</span><span class="chev">${recognizeAssignmentOpen?'−':'＋'}</span></button><div class="recognizeBox ${recognizeAssignmentOpen?'':'hidden'}"><div class="recognizeHead"><div><div class="recognizeTitle">📷 Распознать назначение по фото</div><div class="muted" style="font-size:12px;margin-top:4px">Подходит для листа врача: приложение распознает назначение и курсы лекарств, а вы подтвердите перед сохранением.</div></div></div><div class="grid2" style="margin-top:10px"><input id="aiAssignmentFile" type="file" accept="image/*"><button onclick="aiRecognizeAssignment()" ${aiEnabled?'':'disabled'}>Распознать</button></div>${aiEnabled?'':'<div class="muted" style="font-size:12px;margin-top:6px">ИИ выключен в настройках Railway</div>'}</div><div id="assignmentAiPreview"></div>`}
  async function loadCourses(){
    const root=document.getElementById('coursesBox'); if(!root)return; root.innerHTML='<div class="empty">Загружаю назначения...</div>';
    try{const raw=await api('/api/courses'); const rows=Array.isArray(raw)?raw:(Array.isArray(raw?.items)?raw.items:[]); coursesCache=rows; window.courseGroups={};
      const formHtml=`${recognizeBoxHtml()}<button id="courseAddToggle" class="collapseToggle" onclick="toggleAddAssignmentForm()"><span>${addAssignmentOpen?'Скрыть форму добавления':'Добавить назначение вручную'}</span><span class="chev">${addAssignmentOpen?'−':'＋'}</span></button><div id="courseAddForm" class="courseAddForm ${addAssignmentOpen?'':'hidden'}"><div class="fieldLine"><label>Название</label><input id="courseName" placeholder="Например, назначение гастроэнтеролога"></div><div class="fieldLine"><label>Дата назначения</label><input id="courseDate" type="date"></div><div class="fieldLine"><label>Врач</label><input id="courseDoctor" placeholder="опционально"></div><div class="fieldLine"><label>Комментарий</label><input id="courseComment" placeholder="опционально"></div><div class="fieldLine"><label>Фото / файл</label><input id="courseFile" type="file" accept="image/*,.pdf,.doc,.docx,.txt"></div><button class="wide" onclick="addCourse()" style="margin-top:8px">Сохранить назначение</button></div>`;
      const listHtml=rows.length?rows.map(c=>{const safeName=escapeAttr(c.name||''),safeDoctor=escapeAttr(c.doctor||''),safeComment=escapeAttr(c.comment||'');return `<div class="formrow assignmentCard"><b>${escapeHtml(c.name||'Без названия')}</b><div class="muted" style="font-size:12px;margin-top:3px">Дата назначения: ${escapeHtml(c.assignment_date||'—')}${c.doctor?' · '+escapeHtml(c.doctor):''}</div>${c.comment?`<div class="muted" style="font-size:12px;margin-top:3px">${escapeHtml(c.comment)}</div>`:''}<div class="attachments">${(c.attachments||[]).map(a=>`<button class="fileLinkBtn" onclick="openFilePreview('${attachmentUrl(a.id)}', '${escapeAttr(a.filename)}', ${isImageFilename(a.filename)})">📎 ${escapeHtml(a.filename)}</button>`).join('')}</div>${courseItemsBlock(c)}<div class="adminActions"><button onclick="openCourseMedicineForm(${c.id})">Добавить курс лекарства</button><button class="gray" onclick="aiAddMedicineToCourse(${c.id})" ${aiEnabled?'':'disabled'}>ИИ из текста</button><button class="gray" onclick="editCourse(${c.id}, '${safeName}', '${c.assignment_date||''}', '${safeDoctor}', '${safeComment}')">Изменить назначение</button><button class="danger" onclick="deleteCourse(${c.id})">Удалить назначение</button></div><div class="fieldLine"><label>Добавить файл</label><input id="courseFile_${c.id}" type="file" accept="image/*,.pdf,.doc,.docx,.txt" onchange="uploadCourseFile(${c.id})"></div></div>`}).join(''):'<div class="empty">Назначений пока нет</div>';
      root.innerHTML=formHtml+`<div class="adminList" style="margin-top:14px">${listHtml}</div>`;
  ...
  function scheduleForm(r){const edit=editSchedules.has(r.id);const slot=scheduleSlotText(r);if(edit)return `<div class="scheduleCard pretty"><div class="fieldLine"><label>Назначение</label>${courseSelectHtml(r.id,r.course_id)}</div><div class="scheduleGrid"><input id="s_name_${r.id}" value="${escapeAttr(r.name)}"><input id="s_dose_${r.id}" value="${escapeAttr(r.dose||'')}"><input id="s_time_${r.id}" type="time" value="${r.time_local}"></div><input id="s_label_${r.id}" value="${escapeAttr(r.label||'')}" placeholder="Комментарий"><div class="dateLine"><div><label>Дата начала</label><input id="s_start_${r.id}" type="date" value="${r.start_date||''}"></div><div><label>Дата окончания</label><input id="s_end_${r.id}" type="date" value="${r.end_date||''}"></div></div><div class="adminActions"><button onclick="saveSchedule(${r.id})">Сохранить</button><button class="gray" onclick="cancelScheduleEdit(${r.id})">Отменить</button></div></div>`;return `<div class="scheduleCard pretty"><div class="scheduleMain"><div><div class="scheduleName">${escapeHtml(r.name)}</div><div class="scheduleMetaLine">${escapeHtml(r.dose||'')}${r.label?' · '+escapeHtml(r.label):''}</div></div><div class="scheduleSlot">${escapeHtml(slot)}</div></div><div class="scheduleAssignment">${r.course_id?'из назначения':'без назначения'} · ${r.start_date||'—'} — ${r.end_date||'—'}</div><div class="adminActions"><button class="gray" onclick="editSchedules.add(${r.id});loadSchedules()">Изменить</button><button class="danger" onclick="deleteSchedule(${r.id})">Удалить</button></div></div>`;}
