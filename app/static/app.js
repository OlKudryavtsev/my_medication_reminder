  ...
  function courseGroupBody(g,key){const dp=doseParts(g.dose);const period=`${g.start_date||'—'} — ${g.end_date||'—'}`;const analogs=(g.analogs||'').trim();const route=(g.administration_route||'').trim();const form=(g.dosage_form||dp.unit||'').trim();const inactiveActions = g.active_all ? '' : `<button onclick="startScheduleGroup('${g.ids.join(',')}')">Начать курс</button>`;return `<div id="courseBody_${key}" class="courseGroupBody hidden"><div class="infoGrid"><div class="infoPill"><div class="k">Название</div><div class="v">${escapeHtml(g.name||'—')}</div></div><div class="infoPill"><div class="k">Формат</div><div class="v">${escapeHtml(form||'—')}</div></div><div class="infoPill"><div class="k">Дозировка</div><div class="v">${escapeHtml(g.dose||'—')}</div></div><div class="infoPill"><div class="k">Периодичность</div><div class="v">${escapeHtml(courseFreqText(g))}</div></div><div class="infoPill"><div class="k">Прием</div><div class="v">${escapeHtml(courseApplyText(g))}</div></div><div class="infoPill"><div class="k">Способ применения</div><div class="v">${escapeHtml(route||'—')}</div></div><div class="infoPill"><div class="k">Длительность</div><div class="v">${escapeHtml(durationText(g.start_date,g.end_date,g.duration_value,g.duration_unit))}</div></div><div class="infoPill"><div class="k">Период</div><div class="v">${escapeHtml(period)}</div></div></div>${analogs?`<div class="infoPill" style="margin-top:8px"><div class="k">Аналоги из назначения</div><div class="v">${escapeHtml(analogs)}</div></div>`:''}${courseNeedPanel(g)}<div class="adminActions">${inactiveActions}<button class="gray" onclick="openCourseMedicineForm(${g.course_id}, null, null, '${key}')">Изменить</button><button class="danger" onclick="deleteScheduleGroup('${g.ids.join(',')}')">Удалить</button></div></div>`;}
  ...
  function toggleAssignmentTree(id){ if(expandedAssignments.has(id)) expandedAssignments.delete(id); else expandedAssignments.add(id); loadCourses(); }
  function toggleScheduleTreeGroup(key){ if(expandedScheduleGroups.has(key)) expandedScheduleGroups.delete(key); else expandedScheduleGroups.add(key); loadSchedules(); }
  ...
  async function loadCourses(){
    ...
    try{
      ...
      const list=rows.length?rows.map(c=>{
        ...
        return `<div class="assignmentTreeCard"><div class="assignmentTreeHead" onclick="toggleAssignmentTree(${c.id})"><div><div class="assignmentTreeTitle">${escapeHtml(c.name||'Без названия')}</div><div class="assignmentTreeMeta">${assignmentHeaderMeta(c)} · курсов: ${(c.items||[]).length?assignmentCourseGroups(c).length:0}</div></div><div class="chev">${open?'−':'＋'}</div></div><div class="assignmentTreeBody ${open?'':'hidden'}">${c.comment?`<div class="muted" style="font-size:12px;margin-bottom:6px">${escapeHtml(c.comment)}</div>`:''}<div class="attachments">${(c.attachments||[]).map(a=>`<button class="fileLinkBtn" onclick="openFilePreview('${attachmentUrl(a.id)}', '${escapeAttr(a.filename)}', ${isImageFilename(a.filename)})">📎 ${escapeHtml(a.filename)}</button>`).join('')}</div><div class="treeActions"><button onclick="openCourseMedicineForm(${c.id})">Добавить курс</button><button class="gray" onclick="aiAddMedicineToCourse(${c.id})" ${aiEnabled?'':'disabled'}>Распознать курс из текста</button><button class="gray" onclick="editCourse(${c.id}, '${safeName}', '${c.assignment_date||''}', '${safeDoctor}', '${safeComment}')">Изменить назначение</button><button class="danger" onclick="deleteCourse(${c.id})">Удалить</button></div><div class="fieldLine"><label>Добавить файл</label><input id="courseFile_${c.id}" type="file" accept="image/*,.pdf,.doc,.docx,.txt" onchange="uploadCourseFile(${c.id})"></div><div class="courseItemsTitle">Курсы лекарств</div>${assignmentCoursesTree(c)}</div></div>`;
  ...
  async function loadSchedules(){if(!me?.can_manage_current_profile)return;if(!coursesCache.length){try{coursesCache=await api('/api/courses')}catch(e){coursesCache=[]}}let rows=await api('/api/schedules');const selected=populateMedicineFilter('scheduleSearch', rows, r=>r.name||'');if(selected)rows=rows.filter(r=>(r.name||'')===selected);const root=document.getElementById('schedules');const toolbar=document.querySelector('.scheduleToolbar');if(toolbar){toolbar.innerHTML=`<div class="segmentedFlag"><span class="muted" style="font-size:12px;font-weight:900">Группировка</span></div><div class="scheduleGroupToggle"><button class="${scheduleGroupByAssignmentUi?'active':''}" onclick="scheduleGroupByAssignmentUi=true;loadSchedules()">По назначению</button><button class="${!scheduleGroupByAssignmentUi?'active':''}" onclick="scheduleGroupByAssignmentUi=false;loadSchedules()">Без группировки</button></div>`}if(!rows.length){root.innerHTML='<div class="empty">Активное расписание пустое. Запустите курс в назначении.</div>';return}if(!scheduleGroupByAssignmentUi){root.innerHTML=`<div class="dayGroup">${rows.map(scheduleForm).join('')}</div>`;return}const map=new Map();rows.forEach(r=>{const k=assignmentGroupName(r);if(!map.has(k))map.set(k,[]);map.get(k).push(r)});root.innerHTML=[...map.entries()].map(([name,list])=>{const key='sg_'+name.replace(/[^a-zA-Zа-яА-Я0-9]/g,'_');const open=expandedScheduleGroups.has(key);return `<div class="scheduleTreeGroup"><div class="scheduleTreeHead" onclick="toggleScheduleTreeGroup('${key}')"><span>${escapeHtml(name)}</span><span>${open?'−':'＋'}</span></div><div class="scheduleTreeBody ${open?'':'hidden'}">${list.map(scheduleForm).join('')}</div></div>`}).join('');}
